...
export default function AdoptTree() {
  ...
  const handleConfirmAdoption = async () => {
    ...
    if (loggedUser?.id && loggedUser.id !== undefined && loggedUser.id !== null && loggedUser.id !== 'undefined') {
      try {
        console.log("Intentando refrescar datos del usuario con ID:", loggedUser.id);
      ...
      console.warn("No se pudo refrescar datos del usuario: ID no disponible o inválido");
      ...
      showError("Error: No se pudieron cargar los datos del usuario. Por favor, recarga la página e intenta nuevamente.");
      return;
    }

    // Verificar que tenemos datos válidos del usuario
    if (!currentUser || !currentUser.id) {
      showError("Error: No se pudieron cargar los datos del usuario. Por favor, recarga la página.");
      return;
    }
    ...
    if (Number(currentUser.credits) < Number(tree.price)) {
      showWarning(`No tienes suficientes créditos. Necesitas ${Number(tree.price)} créditos pero solo tienes ${Number(currentUser.credits)}.`);
      return;
    }
    ...
    showConfirm(
      ...
      async () => {
        try {
          ...
          try {
            await refreshUserData();
            console.log("Datos del usuario actualizados desde la base de datos");
          } catch (refreshError) {
            console.error("Error al refrescar datos del usuario:", refreshError);
          ...
          showSuccess("¡Solicitud de adopción enviada exitosamente! Los créditos han sido descontados. Espera la aprobación del administrador.");
          navigate("/home");
        } catch (error) {
          ...
          const errorMessage = error.response?.data?.message || "Error al procesar la adopción. Inténtalo nuevamente.";
      ...
      <div className="adoption-content">
        ...
        <div className="tree-info-section">
          ...
          <div className="tree-card">
            ...
            <div className="tree-details">
              ...
              <p><strong>Precio:</strong> <span className="price">{tree.price} créditos</span></p>
              {tree.description && (
        ...
        <div className="user-info-section">
          ...
          <div className="credits-comparison">
            <p><strong>Costo del árbol:</strong> {Number(tree.price)} créditos</p>
            <p><strong>Tus créditos:</strong> {Number(loggedUser?.credits || 0)} créditos</p>
            <p><strong>Diferencia:</strong> 
              <span className={Number(tree.price) <= Number(loggedUser?.credits || 0) ? "sufficient-credits" : "insufficient-credits-text"}>
          ...
          {Number(tree.price) > Number(loggedUser?.credits || 0) && (
            <div className="insufficient-credits">
              ...
              <p>Necesitas {Number(tree.price) - Number(loggedUser?.credits || 0)} créditos más.</p>
            </div>
          )}
        </div>
        ...
        <div className="action-buttons">
          ...
          <button 
            ...
            disabled={confirming || Number(tree.price) > Number(loggedUser?.credits || 0)}
          >
        ...
        <div className="adoption-info">
          ...
          <ul>
            ...
            <li>Se descontarán {tree.price} créditos de tu cuenta al ser aprobada</li>
