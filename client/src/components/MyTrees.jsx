...
const getIrrigationPricing = (price) => {
  ...
  const discounted = Math.round(original * (1 - IRRIGATION_DISCOUNT_RATE) * 100) / 100;
  const discountAmount = Math.round(original * IRRIGATION_DISCOUNT_RATE * 100) / 100;
...
export default function MyTrees() {
  ...
  const selectedTreePricing = selectedTree ? getIrrigationPricing(selectedTree.price) : null;

  return (
    <div className="mytrees-container">
      {/* Header */}
      <header className="mytrees-header">
                ...
                {editingTree?.id === tree.id ? (
                  ...
                  <div className="name-container">
                    <div className="name-and-status">
                      <h3>{tree.name}</h3>
                      <span className={`status-indicator ${tree.adoptionStatus === 1 ? 'approved' : 'pending'}`}>
              ...
              <div className="tree-info-section">
                ...
                <div className="tree-details-info">
                  ...
                  <div className="detail-item">
                    ...
                    <span className="value">{selectedTree.description || 'Sin descripción'}</span>
                  </div>
                  <div className="detail-item">
                    <span className="label">Dirección:</span>
                    <span className="value">{selectedTree.address || 'Sin dirección'}</span>
                  </div>
                  <div className="detail-item">
                    <span className="label">Precio:</span>
                    <span className="value">
                      <span className="price-original-inline">
                        {formatPoints(selectedTreePricing?.original ?? selectedTree.price)} pts
                      </span>
                    </span>
                  </div>
                  {selectedTreePricing && selectedTreePricing.original > 0 && (
                    <div className="detail-item">
                      <span className="label">Costo para regador (-20%):</span>
                      <span className="value value--discount">
                        <span className="price-discounted">
                          {formatPoints(selectedTreePricing.discounted)} pts
                        </span>
                        <span className="price-saving">
                          Ahorro de {formatPoints(selectedTreePricing.discountAmount)} pts frente al costo base.
                        </span>
                      </span>
                    </div>
                  )}
                  <div className="detail-item">
                    ...
                    <span className={`status-badge status-${selectedTree.adoptionStatus}`}>
                      {selectedTree.adoptionStatus === 1 ? '✅ Aprobado' : '⏳ Pendiente'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="map-section">
                ...
                {selectedTree.latitude && selectedTree.longitude ? (
                  <div className="tree-map">
                    <MapContainer
                      center={[parseFloat(selectedTree.latitude), parseFloat(selectedTree.longitude)]}
                      zoom={15}
                    ...
                    >
                      ...
                      />
                      <Marker
                        position={[parseFloat(selectedTree.latitude), parseFloat(selectedTree.longitude)]}
                      >
                        <Popup>
                          <div className="tree-popup">
                            <h4>{selectedTree.name}</h4>
                            <p><strong>Dirección:</strong> {selectedTree.address}</p>
                            <p><strong>Precio:</strong> {formatPoints(selectedTreePricing?.original ?? selectedTree.price)} pts</p>
                            {selectedTreePricing && selectedTreePricing.original > 0 && (
                              <p><strong>Regador (-20%):</strong> {formatPoints(selectedTreePricing.discounted)} pts</p>
                            )}
                          </div>
                        </Popup>
                      </Marker>
