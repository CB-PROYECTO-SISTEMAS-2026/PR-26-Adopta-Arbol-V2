...
export const approveAdoption = async (req, res) => {
  ...
  try {
    ...
    if (adoptionData.length === 0) {
      ...
      return res.status(404).json({ message: "Adopción no encontrada o ya procesada" });
    }
    ...
    await connection.query(
      "UPDATE adoption SET status = 1 WHERE id = ?",
      [id]
    );
...
export const rejectAdoption = async (req, res) => {
  ...
  try {
    ...
    // Actualizar estado de la adopción
    await connection.query(
      "UPDATE adoption SET status = 0 WHERE id = ?",
      [id]
    );
    ...
    res.json({ message: "Adopción rechazada y créditos devueltos exitosamente" });
...
export const createAdoption = async (req, res) => {
  ...
  try {
    ...
    console.log("Tipos:", { userId: typeof userId, treeId: typeof treeId, status: typeof status });
    ...
    if (treeCheck[0].status !== 1) {
      console.log("ERROR: El árbol no está disponible para adopción. Estado:", treeCheck[0].status);
      await connection.rollback();
      return res.status(400).json({ message: "El árbol no está disponible para adopción" });
    }
    ...
    if (pendingAdoption.length > 0) {
      ...
      await connection.rollback();
      return res.status(400).json({ message: "Ya tienes una solicitud pendiente para este árbol" });
    }
    ...
    console.log("- Créditos del usuario (raw):", userCredits, "(tipo:", typeof userCredits, ")");
    console.log("- Precio del árbol (raw):", treePrice, "(tipo:", typeof treePrice, ")");
    
    // Convertir a números para comparación segura
    const userCreditsNum = Number(userCredits);
    const treePriceNum = Number(treePrice);
    
    console.log("- Créditos del usuario (num):", userCreditsNum, "(tipo:", typeof userCreditsNum, ")");
    console.log("- Precio del árbol (num):", treePriceNum, "(tipo:", typeof treePriceNum, ")");
    ...
    if (userCreditsNum < treePriceNum) {
      ...
      await connection.rollback();
      return res.status(400).json({ message: "No tienes suficientes créditos para adoptar este árbol" });
    }
    ...
    res.status(201).json({
      message: "Solicitud de adopción creada exitosamente y créditos descontados",
